...
mod tests {
    ...
    // The same property as above, generated by `proptest` rather than by the
    // fixed-seed loop, and kept alongside it. The loop's fixed corpus is what
    // makes a report reproducible; this shrinks a failure to the shortest
    // command line that still fails and writes it to `proptest-regressions`,
    // where it becomes a permanent case.
    proptest! {
        #[test]
        fn every_generated_command_line_parses_or_is_refused(raw in generated_command_line()) {
            if let Ok(parsed) = parse_curl_replay(&raw) {
                prop_assert!(!parsed.url.is_empty(), "{raw:?} parsed to an empty url");
                // A header the parser accepted has to have a name; an empty one
                // would reach the request builder as a nameless header.
                for (name, _) in &parsed.headers {
                    prop_assert!(!name.is_empty(), "{raw:?} produced a nameless header");
                }
            }
        }
    }
    ...
    fn generated_command_line() -> impl Strategy<Value = String> {
        prop::collection::vec(prop::sample::select(CURL_ALPHABET), 0..48)
