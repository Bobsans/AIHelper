//! The rule table itself: one entry per file the detector recognises.
//!
//! Data, and nothing else. It lives in its own file because it is 870 lines
//! long and adding an ecosystem means adding one row here - the matcher, the
//! `FileGroup` list and the constructors it calls are all in `rules.rs`, which
//! is what a reader actually has to understand.
//!
//! The table is **ordered**, and the order is published output: a file may
//! match several rules, and `classify_file` returns them in table order. So
//! rows are appended or inserted where they belong, never sorted, and the table
//! is not split by group - the groups interleave on purpose.

use super::*;

pub(super) static RULES: &[FileRule] = &[
