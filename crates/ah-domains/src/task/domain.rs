...
fn run_save(args: super::SaveArgs) -> Result<TaskSaveOutput, AppError> {
    ...
    let updated_unix_seconds = ah_persist::transaction(&store_path, || {
...
fn run_run(args: super::RunArgs, limit: Option<usize>) -> Result<TaskRunOutput, AppError> {
    ...
    let output = crate::run::io::run_command(
        &program,
        &command_args,
        crate::run::io::RunCommandOptions {
            timeout: args
                .timeout_ms
    ...
    let stdout_raw = crate::run::io::render_output(&output.stdout, None);
    let stderr_raw = crate::run::io::render_output(&output.stderr, None);
