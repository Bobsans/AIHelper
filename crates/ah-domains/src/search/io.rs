...
use crate::search::domain::{ContextLine, PatternMatcher, TextCollectStats, TextMatch};
...
fn match_file(config: MatchFileConfig<'_>) -> Result<(Vec<TextMatch>, bool), AppError> {
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        if let Some(column) = crate::search::domain::find_match_column(config.matcher, line) {
