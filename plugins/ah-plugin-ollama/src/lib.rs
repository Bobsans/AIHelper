...
fn execute(cli: OllamaCli, globals: &GlobalOptionsWire) -> InvocationResponse {
    match cli.command {
        OllamaCommand::Ask(args) => execute_ask(args, globals),
        OllamaCommand::Chat(args) => execute_chat(args, globals),
    }
}
