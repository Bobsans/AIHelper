...
release policy requires all three expected archives before publication.
...
`plugins/` directory. For MCP-sensitive changes, also perform a stdio handshake,
inspect `tools/list`, confirm risk metadata, and execute a representative typed
tool call.
