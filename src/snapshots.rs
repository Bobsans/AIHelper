...
/// Every literal that `classify_file` compares against, turned into a path.
///
/// The corpus is derived from the rules rather than guessed: each exact file
/// name, suffix, prefix and path fragment the module tests for appears here,
/// plus the compound cases that need both a name and a location. `rules.rs` has
/// no unit tests of its own and the integration tests assert only that a few
/// ecosystems and tools show up, so this is what pins the classification.
const PROJECT_RULE_FIXTURES: &[(&str, &str)] = &[
